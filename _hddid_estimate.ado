...
program define _hddid_estimate, eclass sortpreserve
    ...
    if `p' > 1 {
        ...
        capture `_hddid_subcmd_prefix' python: ///
            ...
            _sig_generator = bool(_sig_target is not None and callable(_sig_target) and (inspect.isgeneratorfunction(_sig_target) or (_sig_call is not None and inspect.isgeneratorfunction(_sig_call)))); ///
            (_ for _ in ()).throw(TypeError(f"{('_hddid_bridge_call_clime_solve' if _bridge is _sig_target else 'hddid_clime_solve')} must be a synchronous callable, got generator function")) if _sig_generator else None; ///
            ...
            Macro.setLocal("_hddid_py_clime_helper_callable", str((1 if callable(_helper) and not _helper_generator and not _helper_async else 0) if _helper is not None else 0)); ///
            Macro.setLocal("_hddid_py_clime_helper_type", (("generator function" if _helper_generator else ("async generator function" if _helper_asyncgen else ("async function" if _helper_async else type(_helper).__name__))) if _helper is not None else "")); ///
